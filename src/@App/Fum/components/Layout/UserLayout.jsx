...
import { deleteAccount, userInfoMenuMobile, userInfoMenus } from '@App/Fum/configs/menuConfig'
...
const UserLayout = props => {
	...
	const [name, setName] = useState(getDataSession('local', 'fum_user')?.name)
	...
	return (
		<Box className={`mx-auto min-h-[60vh] sm:w-[95%] lg:w-['${maxWidth}px']`} sx={{ maxWidth: `${maxWidth}px` }}>
			...
			>
				<Link to={ROUTER_FUM.event.event_top} className="text-[#00A0E9] sm:text-16 text-14 font-light">
					トップ
				</Link>
				<Link
					to={ROUTER_FUM.user.info}
					className={
						breadcrumbText
				...
				{isVerifyCode ? (
					<Box className="flex">
						<Link
							to={ROUTER_FUM.user.change_email}
