import { ROUTER_FUM, TRANSLATE_FUM } from '../../../configs/constants'
import { Box, Button, IconButton, InputAdornment, Typography } from '@mui/material'
import React, { useEffect, useState } from 'react'
import { useTranslation } from 'react-i18next'
import EventContentPage from '../../../components/Layout/EventContentPage'
import { getFumUser, setDataSession } from '@Core/helper/Session'
...
import iconInquiry from '@App/Fum/assets/iconInquiry.png'
...
const Login = props => {
	...
	const { t } = useTranslation(TRANSLATE_FUM.auth)
					...
					<form onSubmit={onSubmit} className="sm:pt-[56px] pt-[32px] sm:px-0 px-16 sm:pb-[56px] pb-[32px]">
						<Box className="bg-white sm:px-[124px] px-16 ">
							...
							<Box className="justify-center sm:mt-32 mt-[17px] flex ">
								<Typography
									onClick={() => navigate(ROUTER_FUM.auth.email_reset_password)}
									color="primary"
							...
							<Box className="text-center sm:mt-4 mt-8">
								<Button
									...
									fullWidth
									onClick={() => navigate(ROUTER_FUM.inquiry)}
								>
									お問い合わせ
								</Button>
							</Box>
							...
							<Box className="text-center sm:pb-[56px] pb-[24px]">
								<Button
									...
									className="sm:w-[400px] w-full sm:h-56 h-[58px] sm:text-20 text-16 sm:py-12 py-[18px] px-0 sm:leading-[160%] leading-[140%] shadow-none font-semibold text-[#FFFFFF]"
									fullWidth
									onClick={() => navigate(ROUTER_FUM.auth.login_provider)}
								>
									施設提供者アカウントでログインする
								</Button>
							</Box>
						...
						<Box className="bg-white sm:px-[124px] px-16 ">
							...
							<Box className="text-center sm:pb-[56px] pb-[24px]">
								...
								<Button
									...
									disabled={false}
									fullWidth
									onClick={() => navigate(ROUTER_FUM.auth.register.profile)}
								>
									会員登録
								</Button>
								{/* </a> */}
