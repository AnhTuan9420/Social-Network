...
import { clearSession, getFumUser } from '@Core/helper/Session'
import { useBoolean } from 'ahooks'
import { successMsg } from '@Core/helper/Message'
import Person from '@App/Fum/assets/Person.svg'
...
const HeaderPerson = () => {
	...
	const handleLogout = () => {
		...
		navigate(ROUTER_FUM.event.event_top)
		clearSession()
	}
		...
		<div ref={el}>
			...
			>
				...
				<Typography
					...
					onClick={() => navigate(ROUTER_FUM.user.info) & setOpen(!open)}
				>
					マイページ
				</Typography>
				...
				<Typography
					...
					onClick={() => navigate(ROUTER_FUM.questions) & setOpen(!open)}
				>
					よくある質問
				</Typography>
				...
				<Typography
					className="text-[16px] px-16 py-8 text-[#000000] leading-[140%] cursor-pointer "
					onClick={() => navigate(ROUTER_FUM.inquiry) & setOpen(!open)}
				>
					お問い合わせ
				</Typography>
				...
				<Typography
					className="text-[16px] px-16 py-8 text-[#000000] leading-[140%] cursor-pointer "
					onClick={() => navigate(ROUTER_FUM.privacy) & setOpen(!open)}
				>
					プライバシーポリシー
				</Typography>
				...
				<Typography
					className="text-[16px] px-16 py-8 text-[#000000] leading-[140%] cursor-pointer "
					onClick={() => navigate(ROUTER_FUM.cancellation) & setOpen(!open)}
				>
					キャンセルポリシー
				</Typography>
				...
				<Typography
					...
					onClick={() => navigate(ROUTER_FUM.terms) & setOpen(!open)}
				>
					利用規約
				</Typography>
				<Divider />
