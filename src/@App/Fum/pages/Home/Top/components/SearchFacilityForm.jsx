...
import { useSearchDialog } from '@App/Fum/pages/Facility/hooks/useSearchDialog'
	...
	const onSubmit = handleSubmit(data => {
		if (data.facility_type || addTags.length > 0) {
			...
			if (data.facility_type) {
				navigate(ROUTER_FUM.event.search, { state: data?.facility_type })
			} else {
				navigate(ROUTER_FUM.event.search)
			}
		} else {
			navigate(ROUTER_FUM.event.search)
		}
